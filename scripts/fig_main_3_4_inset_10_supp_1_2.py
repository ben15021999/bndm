...
nb_steps = 1000
cov_mat_L = np.load(
    './bluenoise/cov_gaussianBN_L_res{:}_d{:}.npz'.format(64, 3))['x'].astype(np.float32)
cov_mat_L = torch.from_numpy(cov_mat_L).to(device)

cov_mat_L_rn = np.load(
    './bluenoise/cov_gaussianRN_L_res{:}_d{:}.npz'.format(64, 3))['x'].astype(np.float32)
...
def fig_main_inset():
    ...
    plt.legend([r'$\tau={:}$'.format(str(x))
               for x in scheduler_param_list], prop={'size': 15})
...
def fig_main_10():
    ...
    for cur_step in [0]:
        t = torch.randint(low=cur_step, high=cur_step+1,
                          size=(batch_size, )).to(device)
        gamma_t = get_scheduler_gamma(
            t.float(), scheduler_gamma, scheduler_params)
        gaussian_white_noise = torch.randn(
            batch_size, 3, 64, 64).float().to(device)
        gaussian_red_noise, _, _ = get_noise_v2(
            device, gaussian_white_noise, cov_mat_L_rn, gamma_t, t, noise_type=noise_type, train_or_test='test', inplace=True)

        gaussian_red_noise_plot = (gaussian_red_noise - gaussian_red_noise.min()) / (
            gaussian_red_noise.max() - gaussian_red_noise.min())
        ...
        plt.subplot(122)
        plt.imshow(
            gaussian_red_noise_fft[0, 0].real.detach().cpu().numpy(), cmap='gray')
        plt.savefig(
            output_dir+'/gaussianRN_res64_and_spectrum_{:}.png'.format(cur_step))
        plt.clf()
...
def fig_main_3_4():
    ...
    for cur_step in [0, 500, 999]:
        ...
        gaussian_white_noise = torch.randn(
            batch_size, 3, 64, 64).float().to(device)
        gaussian_blue_noise, _, _ = get_noise_v2(
            device, gaussian_white_noise, cov_mat_L, gamma_t, t, noise_type=noise_type, train_or_test='test', inplace=True)
        ...
        plt.subplot(122)
        plt.imshow(
            gaussian_blue_noise_fft[0, 0].real.detach().cpu().numpy(), cmap='gray')
        plt.savefig(
            output_dir+'/gaussianBN_res64_and_spectrum_{:}.png'.format(cur_step))
        plt.clf()
...
def fig_supp_1_2():
    ...
    for repetitive in [True, False]:
        ...
        for realization in tqdm(range(realizations)):
            for res in [128]:
                ...
                gamma_t = get_scheduler_gamma(
                    t.float(), scheduler_gamma, scheduler_params)
                gaussian_white_noise = torch.randn(
                    batch_size, 3, res, res).float().to(device)
                ...
                if repetitive:
                    ...
                    for i in range(num_per_axis):
                        for j in range(num_per_axis):
                            gaussian_white_noise[:, :, i *
                                                 64:(i+1)*64, j*64:(j+1)*64] = patch
                gaussian_blue_noise, _, _ = get_noise_v2(
                    device, gaussian_white_noise, cov_mat_L, gamma_t, t, noise_type=noise_type, train_or_test='test', inplace=True)
        ...
        Image.fromarray((gaussian_blue_noise_plot[0].permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8)).save(
            output_dir+'/gaussianBN_res{:}_and_spectrum_{:}_repetitive_{:}_noise.png'.format(res, cur_step, repetitive))

        avg_gaussian_blue_noise_fft_exr = avg_gaussian_blue_noise_fft[0].permute(
            1, 2, 0).detach().cpu().numpy()[..., 0].astype(np.float32)
        avg_gaussian_blue_noise_fft_exr = avg_gaussian_blue_noise_fft_exr / \
            avg_gaussian_blue_noise_fft_exr.max()
        ...
        cv2.imwrite(output_dir+'/gaussianBN_res{:}_and_spectrum_{:}_repetitive_{:}_spectrum.exr'.format(
            res, cur_step, repetitive), avg_gaussian_blue_noise_fft_exr)
