...
def noise_padding(noise_small, res=128):
    if res == 128:
        ...
        t1, t2, t3, t4 = noise_small[:, 0, ...], noise_small[:,
                                                             1, ...], noise_small[:, 2, ...], noise_small[:, 3, ...]
        if True:
            top_row = torch.cat((t1, t2), dim=-2)     # Concatenate along width
            bottom_row = torch.cat((t3, t4), dim=-2)  # Concatenate along width
            noise = torch.cat((top_row, bottom_row), dim=-
                              1)  # Concatenate along height
    ...
    if noise_type == 'gaussian':
        ...
        if x.shape[-1] == 64:
            ...
            if train_or_test == 'test':
                t1, t2, t3, t4 = x[:, :, 0:64, 0:64], x[:, :, 0:64,
                                                        64:128], x[:, :, 64:128, 0:64], x[:, :, 64:128, 64:128]
                noise_small = torch.cat((t1, t2, t3, t4), dim=0)
                noise_small = noise_small.view(
                    bs * up_scale_sqr, dimension, 64 * 64).permute(0, 2, 1)
        ...
        if x.shape[-1] == 32:
            ...
            noise_bn = torch.matmul(cov_mat_L, noise).permute(
                0, 2, 1).contiguous().view(B, C, H, W)

            if noise_type in ['gaussianBN', 'gaussianRN']:
                noise = noise_bn * (1 - alpha_t.view(-1, 1, 1, 1)) + \
                    noise_wn * alpha_t.view(-1, 1, 1, 1)
            ...
            if inplace:
                ...
                noise_small = torch.randn(
                    bs * up_scale_sqr, dimension, 64, 64).float().to(device)
                # noise = torch.randn_like(x)
            ...
            noise_bn = torch.matmul(cov_mat_L, noise_small).permute(
                0, 2, 1).contiguous().view(bs, up_scale_sqr, dimension, 64, 64)
        ...
        if False:
            ...
            noise_plot = (noise_plot - noise_plot.min()) / \
                (noise_plot.max() - noise_plot.min())
            noise_plot_wn = (noise_plot_wn - noise_plot_wn.min()) / \
                (noise_plot_wn.max() - noise_plot_wn.min())
            ...
            plt.figure(1)
            plt.subplot(121)
            plt.imshow(noise_plot.permute(
                1, 2, 0).detach().cpu().numpy().astype(np.float32))
            plt.subplot(122)
            plt.imshow(noise_plot_wn.permute(
                1, 2, 0).detach().cpu().numpy().astype(np.float32))
            plt.show()
