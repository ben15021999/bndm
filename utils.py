...
def get_scheduler_gamma(x, scheduler, scheduler_params, nb_steps):
    ...
    if scheduler == 'linear':
        ...
        t = x / nb_steps
        output = torch.pow(
            torch.cos((t * (end - start) + start) * np.pi / 2), (2 * tau))
        output = (v_end - output) / (v_end - v_start)
...
def sample_iadb(model, x0, nb_step, scheduler_gamma, scheduler_params, out_channel, noise_type, train_or_test, scheduler_alpha='linear'):
    ...
    for t in seq:
        ...
        gamma_start = get_scheduler_gamma(
            (tt + 1).float(), scheduler_gamma, scheduler_params, nb_step)
        gamma_end = get_scheduler_gamma(
            tt.float(), scheduler_gamma, scheduler_params, nb_step)
        ...
        if noise_type in ['gaussianBN', 'gaussianRN']:
            ...
            if out_channel == 3:
                x_alpha = x_alpha + \
                    (alpha_start - alpha_end).view(-1, 1, 1, 1) * d
            elif out_channel == 6:
                # print('t:', t, gamma_start - gamma_end)
                x_alpha = x_alpha + (alpha_start - alpha_end).view(-1, 1, 1, 1) * d[:, :3, :, :] + (
                    gamma_start - gamma_end).view(-1, 1, 1, 1) * d[:, 3:, :, :]
            else:
                raise NotImplementedError
